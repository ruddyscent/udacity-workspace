...
exec_detection = ['bev_from_pcl', 'detect_objects'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            ...
            plt.imshow(cv2.cvtColor(img_range, cv2.COLOR_BGR2RGB))
            plt.titie('range image')
