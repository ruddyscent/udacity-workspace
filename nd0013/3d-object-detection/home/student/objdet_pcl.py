...
def show_range_image(frame, lidar_name):
    ...
    # step 7: focus on +/- 90 deg. left and right of the forward-facing x-axis
    deg90 = int(img_range_intensity.shape[1] / 4)
    img_range_intensity_center = int(img_range_intensity.shape[1] / 2)
    img_range_intensity = img_range_intensity[:, img_range_intensity_center - deg90:img_range_intensity_center + deg90]
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    _, idx_intensity_unique = np.unique(lidar_top_pcl[:, 0:2], axis=0, return_index=True)
    lidar_top_pcl = lidar_top_pcl[idx_intensity_unique]

    ## step 4 : assign the intensity value of each unique entry in lidar_top_pcl to the intensity map 
    ##          make sure that the intensity is scaled in such a way that objects of interest (e.g. vehicles) are clearly visible    
    ##          also, make sure that the influence of outliers is mitigated by normalizing intensity on the difference between the max. and min. value within the point cloud
    intensity_map[np.int_(lidar_top_pcl[:, 0]), np.int_(lidar_top_pcl[:, 1])] = lidar_top_pcl[:, 2] / float(np.abs(configs.lim_z[1] - configs.lim_z[0]))
    ...
    # height_map = np.clip(height_map * 255, 0, 255).astype(np.uint8)
