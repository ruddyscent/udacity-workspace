...
ChatLogic::~ChatLogic()
{
    ...
    // delete _chatBot;  // _chatBot is not owned.

    // delete all nodes
    // for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    // {
    //     delete *it;  // not required since unique_ptr is used
    // }

    // // delete all edges
    // for (auto it = std::begin(_edges); it != std::end(_edges); ++it)
    // {
    //     delete *it;  //  _edge is not used.
    // }
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // _edges.emplace_back(std::move(edge));  // _edges is not used
    ...
    // create instance of chatbot
    auto chatbot_unique = std::make_unique<ChatBot>("../images/chatbot.png");
    _chatBot = chatbot_unique.get();
    ...
    rootNode->MoveChatbotHere(std::move(chatbot_unique));
